...
import pandas as pd
import os
from Tools import urls, setup_chrome_driver, save_base, Path, thems
from tqdm import tqdm
import time


def get_info_from_cards_RIA(card_urls, id):
    data = {
        "url": [],
        "headers": [],
        "sub_headers": [],
...
def get_info_from_cards_Lenta(card_urls, id):
    data = {
        "ID": [],
        "url": [],
        "headers": [],
    ...
    for url in tqdm(card_urls, desc="Processing", unit="element"):
        try:
            ...
            try:
                data['headers'].append(soup.find(class_="topic-body__title").get_text(strip=True))
            except Exception as e:
                try:
                    data['headers'].append(soup.find(class_="topic-body__title").get_text(strip=True))
                except Exception as e:
            ...
            try:
                data['sub_headers'].append(soup.find(class_="topic-body__title-yandex").get_text(strip=True))
            except Exception as e:
                data['sub_headers'].append(None)

            try:
                text_container = soup.find("div", class_="wtrw topic-body__content")
                data['text'].append(' '.join(p.get_text(strip=True) for p in soup.find_all("p", class_="topic-body__content-text")))
            except Exception as e:
                data['text'].append(None)
...
if __name__ == "__main__":
    ...
    for key in Path.keys():
        ...
        for idx, file in enumerate(files):
            ...
            info_df = get_info_from_cards_Lenta(card_urls, thems[key])
